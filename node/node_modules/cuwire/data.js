...
Arduino.prototype.lookupBoard = function (boardName) {
	var vendor, arch, name, match;
	if (match = boardName.match (/^(\w+)\:(\w+)\:(\w+)$/)) {
		vendor = match[1];
		arch   = match[2];
		name   = match[3];
		return {
			platform: [vendor, arch].join (':'),
			board:    name
		}
	}
	...
	Object.keys (boardData).forEach (function (platformId) {
		Object.keys (boardData[platformId].boards).forEach (function (boardId) {
